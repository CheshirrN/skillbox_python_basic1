...
"""
all_money = 10000#int(input('Введите стипендию: '))
money = 900#int(input('Введите расходы на проживание: '))
no_money = money - all_money

for month in range(10):
    if no_money > 0:
        print(f'{month + 1} месяц траты {money} не хватает {no_money}')
    else:
        print(f'{month + 1} месяц траты {money} остаток {-1 * no_money}')
    if month == 9:
        break
    money += money / 100 * 3
    no_money += money - all_money
...
if no_money > 0:
    print(f'Нужно попросить у родителей {no_money} рублей')
else:
    print(f'осталось неизрасходованно', -1 * no_money, "рублей")
